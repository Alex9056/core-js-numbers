 ...
 *   (0,1) (0,1)     => 0
 */
// The tangent of the angle between two points is defined as delta y / delta x That is (y2 - y1)/(x2-x1).
// This means that math.atan2(dy, dx) give the angle between the two points assuming that you know the base
// axis that defines the co-ordinates.
 ...
 *   10 => 55
 */
// Fibonacci number - следующее число получается как сумма двух предыдущих
function getFibonacciNumber(index) {
  if (index === 0) return 0;
  // return getFibonacciNumber(index - 1) + getFibonacciNumber(index - 2);
  let firstFib = 1;
  let secondFib = 1;
  if (index === 1) return firstFib;
  if (index === 2) return secondFib;
  let fib;
  for (let i = 3; i <= index; i += 1) {
    fib = secondFib + firstFib;
    firstFib = secondFib;
    secondFib = fib;
  }
  return fib;
...
function getSumToN(n) {
  // if (n === 0) return 0;
  // return getSumToN(n) + getSumToN(n - 1);
  let sum = 0;
  for (let i = 0; i <= n; i += 1) {
    sum += i;
  }
...
function getSumOfDigits(num) {
  ...
  for (let i = 1; i <= digits; i += 1) {
    const d = Math.floor((num % 10 ** i) / 10 ** (i - 1));
    sum += d;
  }
  return sum;
}
 ...
 *   15  => false
 */
//  𝙼𝚊𝚝𝚑.𝚕𝚘𝚐𝟸 ( 𝚡 ) = log 2 ( x ) = the unique  y  such that  2**y = x
...
function getCountOfOddNumbers(number) {
  ...
  if (number === 1 || number === 2) return count;
  if (number === -1 || number === -2) return count;
  if (number < 0) {
